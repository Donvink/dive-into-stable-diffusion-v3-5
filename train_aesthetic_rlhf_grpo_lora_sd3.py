#!/usr/bin/env python
# coding=utf-8
# Copyright 2025 Donvink. All rights reserved.
#
...
"""Fine-tuning script for Stable Diffusion 3.5 for text2image with support for RLHF and GRPO."""
...
from src.grpo import GRPOLossCalculator
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Epsilon value for the GRPO optimizers.",
    )
    parser.add_argument(
        "--grpo_beta",
        type=float,
        default=0.1,
        help="Beta value for the GRPO optimizers.",
    )
...
def kl_gaussian_closed_form(latent1: torch.Tensor, latent2: torch.Tensor):
    """
    计算两个高斯分布latent的KL散度（闭式解公式）
    输入维度：(batchsize, channels, H, W)
    假设各通道独立服从高斯分布
    """
    # 计算各通道的均值和标准差（无偏估计设为False）
    mu1 = latent1.mean(dim=[0, 2, 3])  # 沿batch和空间维度求平均，得到channels维的均值
    sigma1 = latent1.std(dim=[0, 2, 3], unbiased=False)
    mu2 = latent2.mean(dim=[0, 2, 3])
    sigma2 = latent2.std(dim=[0, 2, 3], unbiased=False)
...
def main(args):
    ...
    # Load aesthetics predictor
    aesthetics_predictor = AestheticsPredictorV1.from_pretrained(args.aesthetic_predictor_name_or_path)
    ...
    aesthetics_predictor.requires_grad_(False)
    ...
    aesthetics_predictor.to(accelerator.device, dtype=weight_dtype)
    ref_transformer.to(accelerator.device, dtype=weight_dtype)

    aesthetics_predictor.eval()
    ...
    def preprocess_train(examples):
        ...
        examples["prompts"] = examples["text"]
        return examples
    ...
    loss_calculator = GRPOLossCalculator(epsilon=args.grpo_epsilon, beta=args.grpo_beta, scaling_factor=vae.config.scaling_factor)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                shape = (
                    ...
                    int(args.resolution) // vae_scale_factor,   # Height
                    int(args.resolution) // vae_scale_factor,   # Width
                )
                model_input = torch.randn(shape).to(device=prompt_embeds.device, dtype=weight_dtype)
                latents = copy.deepcopy(model_input)
                ref_latents = copy.deepcopy(model_input)
                old_log_probs = torch.distributions.Normal(0, 1).log_prob(latents).sum(dim=(1,2,3))

                # Prepare timesteps
                num_inference_steps = random.randint(10, 20)
                noise_scheduler_copy = copy.deepcopy(noise_scheduler)
                noise_scheduler_copy.set_timesteps(num_inference_steps, device=prompt_embeds.device)
                timesteps = noise_scheduler_copy.timesteps
                ref_noise_scheduler = copy.deepcopy(noise_scheduler_copy)
                # num_warmup_steps = max(len(timesteps) - num_inference_steps * noise_scheduler_copy.order, 0)
                ...
                for i, t in enumerate(timesteps):
                    ...
                    latents = noise_scheduler_copy.step(noise_pred, t, latents, return_dict=False)[0]
                    ref_latents = ref_noise_scheduler.step(ref_pred, t, ref_latents, return_dict=False)[0]
                    ...
                    predictions = outputs.logits.squeeze().tolist()  # Get all scores

                    # Compute rewards
                    rewards = (predictions - 4.5) / 4.5  # Map a score from the range 0-9 to -1 to 1

                    # Compute GRPO Loss
                    loss, policy_loss, kl_penalty, old_log_probs = loss_calculator.compute_loss(
                        latents, ref_latents, old_log_probs, rewards
                    )
    ...
    if accelerator.is_main_process:
        ...
        if args.validation_prompt is not None and args.num_validation_images > 0:
            ...
            # Save images
            images_save_dir = os.path.join(args.output_dir, "validation_images")
            os.makedirs(images_save_dir, exist_ok=True)
            for i, image in enumerate(images):
                image.save(os.path.join(images_save_dir, f"{args.validation_prompt}_{i}.png"))
