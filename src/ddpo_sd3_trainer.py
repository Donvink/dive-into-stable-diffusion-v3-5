...
from .ddpo_sd3_pipeline import DDPOStableDiffusion3Pipeline
...
class DDPOStableDiffusion3Trainer(DDPOTrainer):
    """
    The DDPOStableDiffusion3Trainer uses Deep Diffusion Policy Optimization to optimise diffusion models.
    Note, this trainer is heavily inspired by the work here: https://github.com/kvablack/ddpo-pytorch
    As of now only Stable Diffusion based pipelines are supported

    Attributes:
        **config** (`DDPOConfig`) -- Configuration object for DDPOStableDiffusion3Trainer. Check the documentation of `PPOConfig` for more
         details.
        **reward_function** (Callable[[torch.Tensor, tuple[str], tuple[Any]], torch.Tensor]) -- Reward function to be used
        **prompt_function** (Callable[[], tuple[str, Any]]) -- Function to generate prompts to guide model
        **sd_pipeline** (`DDPOStableDiffusion3Pipeline`) -- Stable Diffusion pipeline to be used for training.
    ...
    def __init__(
        ...
        sd_pipeline: DDPOStableDiffusion3Pipeline,
        ...
        self.sd_pipeline.text_encoder_3.to(self.accelerator.device, dtype=inference_dtype)
        ...
        self.neg_prompts = [""] if self.config.negative_prompts is None else self.config.negative_prompts,
    ...
    def step(self, epoch: int, global_step: int):
        ...
        if self.image_samples_callback is not None and len(self.accelerator.trackers) > 0:
    ...
    def calculate_loss(self, latents, timesteps, next_latents, log_probs, advantages, embeds, pooled_prompt_embeds):
        """
        ...
        with self.autocast():
            if self.config.train_cfg:
                noise_pred = self.sd_pipeline.transformer(
    ...
    def _generate_samples(self, iterations, batch_size):
        ...
        samples = []
        prompt_image_pairs = []
        self.sd_pipeline.transformer.eval()
        ...
        for _ in range(iterations):
            ...
            negative_prompts = [self.neg_prompts[0]] * len(prompts)
            (
                prompt_embeds,
    ...
    def _train_batched_samples(self, inner_epoch, epoch, global_step, batched_samples):
        ...
        for _i, sample in enumerate(batched_samples):
            ...
            for j in range(self.num_train_timesteps):
                with self.accelerator.accumulate(self.sd_pipeline.transformer):
    ...
    def save_model(self, output_dir):
        if self.accelerator.is_main_process:
            transformer = self.unwrap_model(self.sd_pipeline.transformer)
